...
class PeliculasAccesoDatos
{
	...
	function obtenerDatos($idPelicula)
	{
		...
		if (mysqli_connect_errno())
		{
 		...
 		mysqli_select_db($conexion, 'Peliculas');
		$consulta = mysqli_prepare($conexion, "select ID,titulo,año,duracion,sinopsis,imagen,votos from T_Peliculas WHERE ID = '".$idPelicula."'");
        $consulta->execute();
        $result = $consulta->get_result();
	...
	function obtenerCategorias()
	{
		...
		if (mysqli_connect_errno())
		{
 		...
 		mysqli_select_db($conexion, 'Peliculas');
		$consulta = mysqli_prepare($conexion, "select T_Categorias.ID,nombre from T_Categorias");
        $consulta->execute();
        $result = $consulta->get_result();
